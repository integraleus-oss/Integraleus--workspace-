...
Если пользователь просит проект на Alpha Platform, не считать готовым проектом
статичную мнемосхему, WebViewer-картинку или ZIP с неподнятыми companion
артефактами. До передачи результата обязательно разделить и проверить уровни:

- Alpha.HMI screen/package: `.hmi/.omobj` компилируются и открываются;
- Alpha runtime config: Alpha.Server/Historian/HMI.Alarms/Reports/Security/
  Imitator настроены или явно отмечены как недоступные/заготовки;
- live operator runtime: команда с HMI записывает тег, есть readback/статус,
  interlock/role/quality state обработаны, а WebViewer показывает тот же runtime.

Для любого управляемого оборудования минимум одна команда должна быть
доказана end-to-end: click/button -> command tag/write -> simulator/server
state change -> HMI readback. Если это не доказано, результат называть только
экраном, импорт-пакетом, спецификацией или демо, но не полноценным проектом
Alpha Platform.

Жёсткое правило соответствия ТЗ: если в любом техническом задании на систему
АСУ ТП, SCADA/HMI или диспетчеризацию написано сделать проект средствами
программного продукта Альфа платформа, то весь заявленный проект должен быть
реализован средствами Альфа платформы и по документированному Alpha workflow,
включая серверную часть проекта и HMI. Внешние HTML/JS/API-страницы допустимы
только как диагностика или тестовый harness; они не засчитываются как
операторские органы управления, архив/тренды/аварии/отчёты/безопасность,
серверная конфигурация или приёмочное evidence для проекта Alpha Platform.
Нельзя закрывать внешней обвязкой требования вроде пуска/остановки насосов,
ручного/автоматического режима, выбора и записи уставок, аварий, архива,
трендов, отчётов, ролей, аудита, серверной части проекта или интеграции с
имитатором.
