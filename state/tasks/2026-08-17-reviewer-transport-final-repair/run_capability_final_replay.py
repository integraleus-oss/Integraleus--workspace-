...
PROJECT = Path(
    ...
    "manual-pilot-001-capability-grant"
)
...
ATTEMPT_TWO = REVIEW_ROOT / (
    "manual-pilot-001-capability-grant-rework-targeted-review/live-review/"
    "policy-runs/run_home-agent-factory-manual-pilot-001.attempt-2"
)
ATTEMPT_ONE_INPUTS = (
    REVIEW_ROOT / "manual-pilot-001-capability-grant-rework-review/inputs"
)
OUTPUT = REVIEW_ROOT / (
    "manual-pilot-001-capability-grant-rework-final-review-transport-replay-r2"
)
TASK = WORKSPACE / "state/tasks/2026-08-17-orchestrator-manual-pilot-001"
PREVIOUS = WORKSPACE / "state/tasks/2026-08-17-capability-grant-model-policy-rework"
EXPECTED_DIFF = "5b29fb1e2a91f34c9a1c01c8ae04734a4f289ab4dcc29ecaff07e8e8dcf17fa2"
...
def main() -> int:
    if OUTPUT.exists():
        raise RuntimeError("final replay output already exists")
    ...
    prior_context = {
        ...
        "prior_attempts": [
            {**decision_summary(decision_one), "attempt_epoch": 1}
        ],
    ...
    actual_diff = read(built["input_dir"] / "evidence.json")["artifacts"][
        "diff"
    ].removeprefix("sha256:")
    if actual_diff != EXPECTED_DIFF:
        raise RuntimeError(
            f"capability diff changed: expected {EXPECTED_DIFF}, got {actual_diff}"
        )
    anchored = file_digest(built["seal"])

    prompt = OUTPUT / "review-prompt.md"
    prompt.write_text(
        (PREVIOUS / "FINAL_REVIEW_PROMPT.md").read_text(encoding="utf-8")
    ...
    summary = {
        "document_type": "capability_grant_final_review_replay_result",
