...
expected_project=/home/stanislav/agent-runs/orchestrator-worktrees/alpha-bpr-universal-integration-r4-closure
expected_fixture="$expected_project"
expected_artifact=/tmp/alpha-bpr-universal-integration-r4-closure-artifacts
...
case "$mode" in
  ...
  focused) run_tests 'FullyQualifiedName~Integration' 27 focused_integration_tests ;;
  build) dotnet build "$project_root/AlphaBpr.sln" --no-restore "${common[@]}" ;;
  baseline) run_tests '' 365 baseline_regression_tests ;;
  full) run_tests '' 367 full_regression_tests ;;
