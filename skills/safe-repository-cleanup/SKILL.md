...
2. Classify every candidate as durable source, durable evidence/state, generated output, backup/migration remnant, or ambiguous; leave durable and ambiguous paths untouched, and when a semantic batch contains no proven disposable targets, record that result and proceed without creating an unnecessary archive, completing the step when the cleanup list contains only proven temporary or generated files or is explicitly empty.
...
6. Keep message queues, notification records, SQLite data, and delivery state out of repository cleanup unless separately scoped and approved; never replay or resend messages as a cleanup side effect, completing the step when evidence confirms those systems were not mutated.
