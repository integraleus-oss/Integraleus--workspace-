...
STATUS: ACTIVE
DATE: 2026-08-15
TITLE: Local orchestrator returns to controlled manual readiness after pilot rework
CONTENT: Treat local commit `9d43129` (`fix: close orchestrator pilot blockers`) as the completed bounded rework of the controlled manual pilot. The orchestrator status is `READY FOR CONTROLLED MANUAL USE; NOT ACTIVATED`. Complete sealed prior-finding records now survive targeted re-verification, and operator interruption during both Codex implementation and Claude review produces terminal `INTERRUPTED` evidence with exit code `130`, no policy continuation, and no orphaned pilot process. The fresh T02 rerun reached `ACCEPTED / R17_ACCEPT`; targeted replay carried and reprocessed all three original findings; live interruption drills covered both implementation and review legs. Verification passed 94/94 integration tests, 87/87 core contract tests, Python compilation, whitespace checks, and final independent review `ACCEPT`. This decision authorizes only foreground, manually initiated, bounded tasks with explicit task packets and supervision. It does not authorize OpenClaw integration, Gateway/runtime/config changes, cron, systemd/daemon, unattended execution, automatic commit, push, deploy, or source-project transfer.
RATIONALE: Stanislav explicitly approved the Memory Candidate in Telegram topic `HOME:2922` on 2026-08-15 after reviewing the rework report, commit `9d43129`, successful T02 rerun and targeted replay, structured Codex- and Claude-leg interruption evidence, clean source repository, complete test results, and independent acceptance review.
...
STATUS: ACTIVE
DATE: 2026-08-16
TITLE: Accepted manual-004 external-read contract transferred to home-agent-factory
CONTENT: Treat local `home-agent-factory` commit `6e56761` (`fix: harden external read policy contract`) as the controlled source transfer of the accepted `manual-004` result. The transferred scope is exactly `schemas/agent-pack.schema.json`, `src/core/policy.js`, and `test/policy.test.js`; all three files matched the accepted detached-worktree versions byte-for-byte. The runtime and schema now share one pinned `externalRead` grammar, malformed values fail closed with `PolicyError`, GET remains the only method, domain/IP values are canonicalized consistently, and compiled grant containers including nested `modelPolicy` are detached and deeply immutable. Repeated source verification passed 12/12 tests and `git diff --check`; the target repository was clean after the local commit. This decision does not authorize push, deploy, OpenClaw/Gateway integration, cron, systemd/daemon, unattended execution, or automatic future source transfers. The orchestrator remains `READY FOR CONTROLLED MANUAL USE; NOT ACTIVATED`.
RATIONALE: Stanislav explicitly approved the Memory Candidate in Telegram topic `HOME:2922` on 2026-08-16 after the separate controlled transfer completed, the three accepted files were verified byte-for-byte, 12/12 source tests and whitespace checks passed, local target commit `6e56761` was created, and the evidence was recorded without push, deploy, or activation.
...
STATUS: ACTIVE
DATE: 2026-08-16
TITLE: Manual-005 model-class hardening ends in fail-closed escalation
CONTENT: Treat controlled task `manual-005` as terminal `ESCALATED`; its detached-worktree diff is not accepted and must not be transferred to source. One Codex implementation changed only `src/core/policy.js` and `test/policy.test.js`; managed and independently repeated checks passed 18/18 tests and `git diff --check`. The initial Claude review stated that AC-1 through AC-5 were satisfied and reported no blocker or major, but the bounded repaired verdict remained semantically invalid because `command_output` evidence omitted its required `command` block. The orchestrator correctly ended fail-closed with exit code 4, did not launch a second Codex implementation, did not accept the diff, left no orphan processes, and kept source `home-agent-factory` clean at `6e56761`. A fresh bounded task is required before any model-class change can be transferred. The orchestrator remains `READY FOR CONTROLLED MANUAL USE; NOT ACTIVATED`.
RATIONALE: Stanislav explicitly approved the Memory Candidate in Telegram topic `HOME:2922` on 2026-08-16 after reviewing the terminal escalation, 18/18 passing tests, clean changed-path boundary, invalid sealed Claude verdict, unchanged source repository, and evidence commit `3932bf0`.
...
STATUS: ACTIVE
DATE: 2026-08-16
TITLE: Accepted manual-006 model-class contract transferred to home-agent-factory
CONTENT: Treat local `home-agent-factory` commit `f25e001` (`fix: align model class runtime and schema`) as the controlled source transfer of the accepted `manual-006` result. The transferred scope is exactly `schemas/agent-pack.schema.json`, `src/core/policy.js`, and `test/policy.test.js`; all three files matched the accepted detached-worktree versions byte-for-byte. Runtime and agent-pack JSON Schema now use one exported deeply frozen model-class grammar containing exactly `local` and `cloud`; consulted invalid values fail with controlled `PolicyError` before approval logic, source precedence and undefined-as-absent semantics are pinned, and the complete modelPolicy schema subtree is checked against the runtime grammar. The managed cycle reached `ACCEPTED / R17_ACCEPT` after one Codex implementation and a review-only final-full leg; final review reported zero blocker, zero major, and four non-blocking nits. Repeated source verification passed 24/24 tests and `git diff --check`; the target repository was clean after local commit. This decision does not authorize push, deploy, OpenClaw/Gateway integration, cron, systemd/daemon, unattended execution, or automatic future source transfers. The orchestrator remains `READY FOR CONTROLLED MANUAL USE; NOT ACTIVATED`.
RATIONALE: Stanislav explicitly approved the Memory Candidate in Telegram topic `HOME:2922` on 2026-08-16 after reviewing `R17_ACCEPT`, the controlled byte-for-byte transfer, 24/24 passing source tests, clean three-file boundary, local source commit `f25e001`, evidence commit `d42f66d`, and the absence of push, deploy, or activation.
...
STATUS: ACTIVE
DATE: 2026-08-16
TITLE: Manual-007 run-log hardening accepted and ready for separate transfer
CONTENT: Treat controlled task `manual-007` as `ACCEPTED / R17_ACCEPT`, with its unchanged detached-worktree diff ready only for a separately authorized source transfer. The accepted scope is exactly `schemas/run-log.schema.json`, `src/core/run-log.js`, and `test/run-log.test.js`. The implementation validates and serializes run-log input before filesystem mutation, constrains `runId` and event grammar, aligns runtime validation with the run-log Schema, writes parseable newline-terminated JSONL to the bounded runs path, and proves rejected writes have no filesystem side effects. The original and final-full sealed review subjects matched at SHA-256 `797e4e7406f3dae3c6134275881fd3ee512906e40c0921b3b3c7e4ae58142295`; sealed gates passed 33/33 tests and `git diff --check`. After the original Claude timeout, a sealed review-only retry reached `R15_NEED_FULL_REVIEW`; the mandatory fresh final-full leg then reached terminal `R17_ACCEPT` with zero blocker, zero major, and nine non-blocking nits. Source `home-agent-factory` remains clean at `f25e001`; this decision does not itself authorize transfer, source commit, push, deploy, OpenClaw/Gateway integration, cron, systemd/daemon, unattended execution, or automatic future transfers. The orchestrator remains `READY FOR CONTROLLED MANUAL USE; NOT ACTIVATED`.
RATIONALE: Stanislav explicitly approved the Memory Candidate in Telegram topic `HOME:2922` on 2026-08-16 after reviewing the admitted initial-full and final-full review chain, terminal `R17_ACCEPT`, unchanged sealed diff, 33/33 passing tests, clean source boundary, evidence commit `07997ea`, and the proposed separate transfer gate.
...
STATUS: ACTIVE
DATE: 2026-08-16
TITLE: Controlled manual-use evidence supports a limited OpenClaw pilot decision gate
CONTENT: Close the local Codex-Claude orchestrator's extended controlled-manual-use assessment with status `READY FOR A SEPARATELY APPROVED CONTROLLED MANUAL OPENCLAW PILOT; NOT ACTIVATED; NOT READY FOR UNATTENDED USE`. Across `manual-001` through `manual-009`, five tasks reached `ACCEPTED / R17_ACCEPT` and four stopped fail-closed as `ESCALATED`. Accepted manual-004, manual-006, and manual-007 were transferred only through separate controlled source steps; manual-007 is committed in `home-agent-factory` as `8985b8e` after byte-for-byte verification and 33/33 tests. Manual-008 reached `R17_ACCEPT` with 44/44 tests and remains isolated pending a separate transfer. Manual-009 passed 37/37 tests but is prohibited from transfer: review found a major runtime/schema mismatch and the repaired reviewer contract remained invalid. The evidence demonstrates strong safety and engineering usefulness but only moderate-to-poor efficiency because reviewer timeouts and exact-contract repairs remain operationally expensive. A future OpenClaw pilot, if separately approved, must be a thin foreground manual trigger for one packetized task at a time, with fixed repository/worktree, explicit paths/gates/budgets/timeouts, no automatic source transfer/commit/push/deploy/external send, no cron/daemon/background continuation, structured terminal status, and manual approval for every subsequent transfer or higher-risk action. This decision records readiness evidence only and does not authorize OpenClaw runtime/config/Gateway changes or activation.
RATIONALE: Stanislav explicitly approved the Memory Candidate in Telegram topic `HOME:2922` on 2026-08-16 after reviewing the nine-task assessment, five accepted and four fail-closed outcomes, source commit `8985b8e`, accepted-but-untransferred manual-008, prohibited manual-009 diff, clean source boundary, assessment evidence commit `057c812`, and the recommendation for a separately approved foreground-only OpenClaw pilot.
...
STATUS: ACTIVE
DATE: 2026-08-17
TITLE: Trust local OpenClaw for controlled manual orchestration and freeze OS guard work
CONTENT: Treat the current administrator and OpenClaw runtime as trusted for foreground controlled-manual orchestration because Stanislav is the only person with access and the machine is local to the home network. Freeze the uninstalled root-guard/Gateway-plugin work as a research prototype; do not install its system user, `/opt` tree, systemd units, plugin, or Gateway changes. Controlled use remains one packet at a time in a detached worktree, with fixed allowed paths, bounded Codex/Claude attempts and timeouts, tests, sealed evidence, fail-closed admission, and no automatic source transfer, commit, push, deploy, cron, or unattended continuation. Revisit OS-level separation before multi-user/external access, unattended operation, or automatic high-impact actions. The first explicitly approved OpenClaw manual pilot (`manual-pilot-001`) ran from Telegram message `3321` and ended safely `ESCALATED`: one implementation changed exactly three allowed files and passes 46/46 tests plus `git diff --check`, but the repaired Claude verdict remained contract-invalid (`test_result` evidence lacked `command`), so the diff is not accepted and transfer is prohibited.
RATIONALE: Stanislav stated in Telegram topic `HOME:2922` that only he has access and the host is a local machine on the home network, then instructed the agent to execute all proposed simplification and pilot steps. This materially narrows the threat model and makes the proposed OS boundary disproportionate for present manual use; the pilot simultaneously confirmed that existing sealed admission still fails closed on invalid reviewer evidence.
...
STATUS: ACTIVE
DATE: 2026-08-20
TITLE: Require deterministic task-harness preflight before sealed orchestrator runs
CONTENT: Before every sealed controlled-manual orchestrator RUN, execute every task-specific harness repeatedly against the exact fixed baseline and inputs. Admission requires deterministic results across the preflight series; a flaky or state-dependent harness blocks sealing and must be corrected first. Avoid `producer | grep -q` membership checks under `set -o pipefail`, because early consumer exit can produce SIGPIPE and false failure. Bind authoritative packet, harness, worktree, run-root, and output locations with absolute paths. A failure caused solely by packet or harness infrastructure must not be represented as a product defect. Review-only closure may reuse frozen outputs only when their digest is unchanged, the corrected harness passes repeatedly, and an independent closure review reports zero blocker and zero major.
RATIONALE: Stanislav explicitly approved the Memory Candidate in Telegram topic `HOME:2922` on 2026-08-20 after the Alpha BPR Track F release-audit pilot exposed a path-binding failure in R1 and a nondeterministic `printf | grep -q` plus `pipefail` failure in R2. The corrected closure harness passed 10/10, the frozen output digest remained `97349e91b94762f384a924555c2022841854b625f4eadbd991744860e8739912`, and independent closure review returned `ACCEPTED` with 0 blocker and 0 major.
...
STATUS: ACTIVE
DATE: 2026-08-21
TITLE: Trusted Test Broker and bounded builder repair for the orchestrator .NET profile
CONTENT: Make the Trusted Test Broker execution adapter, typed gate-failure classification, deterministic execution-context paths, and one bounded builder-repair attempt mandatory architecture for the local orchestrator's .NET profile. The operator-facing flow remains `PREPARE` then `RUN`. The broker is not an OS security boundary: it executes only operator-sealed allowlisted commands, but those commands may compile or run agent-authored test code under the trusted local-administrator threat model. Before trusted gates and blind acceptance, agent-writable external artifact roots must be cleared; gate processes receive a scrubbed minimal environment without inherited API keys or SSH agent state. Repairable exit-1 implementation gate failures may produce one bounded authenticated repair directive whose embedded gate output is explicitly untrusted diagnostic data. Timeout, other nonzero exits, test-count mismatch, scope violations, ignored-state mutations, unsafe gate definitions, and exhausted repair budgets remain fail-closed. The single existing rework budget is shared: if builder repair consumes it, a later reviewer rework escalates rather than expanding the cycle. Runtime nondeterminism detection is not yet part of the broker, so repeated deterministic harness proof remains mandatory during PREPARE. Do not resume Alpha BPR product pilots until the relevant .NET packet passes these qualification gates; transfer, commit, push, deploy, Gateway/runtime activation, and unattended use remain separate approval gates.
RATIONALE: Stanislav explicitly approved the Memory Candidate in Telegram topic `HOME:2922` on 2026-08-21 after the architecture was derived with a TRIZ-style contradiction analysis and implemented as a minimal internal slice. Verification passed the full 171/171 orchestrator regression suite, a controlled `implementation failure -> trusted builder evidence -> authenticated repair -> gates pass -> review` fixture, real external-artifact .NET build through the broker after per-gate HOME/TMPDIR reset, Python compilation, and `git diff --check`, without an Alpha BPR product run, transfer, commit, push, or deploy.
---
### ID: D-2026-08-22-01
TYPE: DECISION
STATUS: ACTIVE
DATE: 2026-08-22
TITLE: Close Alpha BPR TRIZ P3 and proceed to Track E read-first
CONTENT: Treat Alpha BPR TRIZ P3 as completed at local commit `061fcc8 feat: add P3 recipe approval proof summary`. The accepted increment provides one visible reviewer proof-summary for lifecycle, audit, ontology, diff, report readiness, and mode/source, with fail-closed UI/BFF approval guards. Verification passed 27/27 P3 tests, 367/367 full tests, a zero-warning/zero-error build, Chromium reviewer smoke, and independent review with 0 blocker and 0 major. Push and deploy were not performed. The next Alpha BPR stage is Track E read-first plan/fact and deviation lane.
RATIONALE: Stanislav explicitly approved this Memory Candidate in Telegram topic `HOME:14` on 2026-08-22 after receiving the implementation, verification, independent-review, commit, and clean-worktree result.
...
STATUS: ACTIVE
DATE: 2026-08-22
TITLE: Accept Alpha BPR read-only Integration Wizard and browse
CONTENT: Treat Alpha BPR roadmap stage 5, the read-only Integration Wizard/browse vertical, as completed at local commit `f21247a feat: add read-only integration wizard`. The accepted path exposes connection, bounded OPC UA browse, explicit mapping, compiled validation, compiled gateway dry-run, and one-use activation of the exact successfully dry-run portable Customer Integration Profile. Only user-readable OPC UA Variables are selectable; runtime endpoints, credentials, and security settings remain outside the portable profile and UI. Curated/public mode exposes no bindings or actions, live operations require a dedicated Technologist credential, and write, method, command, setpoint, deployment, and runtime-mutation surfaces remain absent. Verification passed 52/52 focused Integration Wizard + Track E + P3 tests, Python compilation, `git diff --check`, prior full unit/integration/build/browser gates, and final independent review with 0 blocker and 0 major after all major findings were corrected. Push and deploy were not performed. The next product vertical is stage 6: browse-assisted generation of Customer Integration Profile drafts, retaining human confirmation and the read-only boundary.
RATIONALE: Stanislav explicitly approved this Memory Candidate in Telegram topic `HOME:14` on 2026-08-22 after the reviewed increment was committed as `f21247a`, the worktree was reported clean, and the next boundary was identified as Integration Profile draft generation from browse results.
...
STATUS: ACTIVE
DATE: 2026-08-22
TITLE: Accept automatic versioned RUN_EVIDENCE for controlled orchestrator runs
CONTENT: Treat the automatic `RUN_EVIDENCE.jsonl` increment as accepted for the local controlled-manual orchestrator. Every admitted production run now creates a schema-versioned `1.0.0`, append-only, monotonically sequenced and SHA-256 hash-chained evidence stream covering start, agent launches, changed paths, gates, review decisions, and one terminal outcome. Existing or symlinked run roots, malformed or externally modified streams, post-terminal appends, secret-like payload keys, and unsupported JSON values fail closed. Error and interrupt paths preserve the original exception even when terminal evidence or stderr reporting fails. Final verification passed 57/57 focused tests, 186/186 full orchestrator tests, and `git diff --check`; independent closure review returned `ACCEPTED` with zero blocker and zero major. Evidence is recorded in `state/tasks/2026-08-22-orchestrator-run-evidence/EVIDENCE.md`; final evidence commit is `e0b7b9de`. Push, deploy, Alpha BPR changes, `home-agent-factory` changes, Gateway/runtime activation, and unattended use were not authorized. The next orchestrator increment is programmatically constrained `review-only`, `safe-fix`, and `sealed-pilot` profiles.
RATIONALE: Stanislav explicitly approved the Memory Candidate in Telegram topic `HOME:2922` on 2026-08-22 after reviewing the accepted automatic evidence lifecycle, final 57/57 and 186/186 test results, zero-blocker/zero-major independent review, evidence location, local commits, and unchanged product repositories.
...
STATUS: ACTIVE
DATE: 2026-08-25
TITLE: Use a second isolated Alpha.Server for online Universal Recipe Testbed process simulation
CONTENT: The Universal Recipe Testbed live command path will use two isolated Alpha.Server runtimes connected through native Alpha.Link (`HUB Module -> TCP Server`): one HMI-facing command/readback server and one online process-simulator server that owns the fail-closed handshake. Alpha.Imitator remains reserved for its supported history-driven simulation sessions and is not an always-online command executor. This supersedes the runtime assumption in `D-2026-08-25-02` that Alpha.Imitator could host the live handshake, while retaining the typed contract and native transport intent. A controlled local probe proved the HUB/TCP connection and Server OPC UA ingress but produced `BadWaitingForInitialData` from Alpha.Imitator outside a history session. Existing services remained active and unchanged; production, PLC and customer systems remain excluded.
RATIONALE: Stanislav explicitly approved the Memory Candidate and architecture change in Telegram topic `HOME:14` on 2026-08-25 after receiving the isolated runtime findings and rollback report.
---
### ID: D-2026-08-25-07
TYPE: DECISION
STATUS: ACTIVE
DATE: 2026-08-25
TITLE: Accept Universal Recipe Testbed Phase 4 data-driven batch execution
CONTENT: Treat `/home/stanislav/projects/alpha-bpr-universal-testbed` commit `8a32d3a` (`feat: execute data-driven recipe batches`) as the accepted Phase 4 vertical-slice baseline. One compiled two-Alpha.Server runtime connected through native Alpha.Link executes all three canonical recipes from input data without rebuilding between recipes. The accepted behavior covers pause/resume, operator approval, abort/reset, monotonic audit sequence, fail-closed invalid recipe/version/operation/parameter handling, the complete Phase 3 negative regression matrix, and real Alpha.HMI Viewer START/RESET clicks with independent OPC UA readback. `./scripts/run_live_gate.sh` reproduces generation, native compile/rebuild, HMI BINOM export, live tests, Viewer clicks, rollback, and service-state verification in one controlled command. Isolated runtimes are stopped after the gate; existing `alpha.server`, `alpha.imitator`, and `alpha.domain` remain active. No push, PLC, customer, or production change was performed. The next product increment is Phase 5: the native Alpha.HMI screen hierarchy.
RATIONALE: Stanislav explicitly approved the Phase 4 Memory Candidate in Telegram topic `HOME:14` on 2026-08-25 after receiving the three-recipe execution result, repeatable live-gate proof, rollback status, clean worktree, and commit `8a32d3a`.
---
### ID: D-2026-08-25-08
TYPE: DECISION
STATUS: ACTIVE
DATE: 2026-08-25
TITLE: Accept Universal Recipe Testbed Phase 5 native Alpha.HMI hierarchy
CONTENT: Treat `/home/stanislav/projects/alpha-bpr-universal-testbed` commit `9da2c84` (`feat: add native hmi screen hierarchy`) as the accepted Phase 5 baseline. The native Alpha.HMI application provides four genuinely navigable levels: L1 installation overview without process commands, L2 reactor and batch control, L3 equipment/recipe/batch detail, and L4 native transport and command diagnostics. Real Alpha.HMI Viewer navigation L1-L4, L2 START/RESET clicks with independent OPC UA readback, the three-recipe Phase 4 matrix, negative command scenarios, Alpha.DevStudio compile/rebuild and Alpha.HMI BINOM export all passed through the repeatable `./scripts/run_live_gate.sh` gate. Screenshot review found and corrected right-edge clipping at 1280x900. Isolated runtimes were stopped, test ports closed, and existing `alpha.server`, `alpha.imitator`, and `alpha.domain` remained active. Alpha.HMI.Alarms, alpha.hmi.charts/Alpha.Historian, Alpha.Security and Alpha.Reports remain an explicit Phase 6 boundary; no push, PLC, customer or production change was performed.
RATIONALE: Stanislav explicitly approved the Phase 5 Memory Candidate in Telegram topic `HOME:14` on 2026-08-25 after receiving the completed hierarchy, real Viewer/readback verification, rollback and service status, clean worktree, and commit `9da2c84`.
---
### ID: D-2026-08-25-09
TYPE: DECISION
STATUS: ACTIVE
DATE: 2026-08-25
TITLE: Accept the third hosted WordPress production release
CONTENT: Treat the third hosted WordPress vertical, «КИИ и российские SCADA: что проверить до выбора платформы», as successfully published and verified in production. The accepted release source is `spectech-sites` commit `1e5aa54`; production evidence is recorded in commit `02425ed`. A complete pre-release backup preserved five existing files and recorded six absent targets for rollback. Exactly 11 manifest files were uploaded through pinned FTPS, and remote SHA-256 matched local sources for all 11/11 files. All target URLs returned HTTP 200; both article variants have correct canonical, `index, follow`, and Article JSON-LD; both cards and sitemap entries are present; PNG/PDF assets have correct MIME types; referenced CSS and control URLs passed; the hosted CMS remained protected. Deprecated Alpha names were absent and rollback was not required. `/lk/**`, forms, PHP/MySQL, WordPress, uploads, DNS, Synology, and non-manifest production files were not changed.
RATIONALE: Stanislav explicitly approved this Memory Candidate in Telegram topic `HOME:990` on 2026-08-25 after separately authorizing the production release and receiving the complete verification report.
---
### ID: D-2026-08-25-10
TYPE: DECISION
STATUS: ACTIVE
DATE: 2026-08-25
TITLE: Accept Universal Recipe Testbed Phase 6 native operations
CONTENT: Treat `/home/stanislav/projects/alpha-bpr-universal-testbed` commit `65d5f87` (`feat: add native phase 6 operations`) as the accepted Phase 6 baseline. The repeatable `./scripts/run_phase6_gate.sh` proof uses current native Alpha modules and verifies an active Alpha.HMI.Alarms condition with acknowledgement readback, Alpha.Historian storage with 102 archived values and 25 distinct process values, embedded alpha.hmi.charts live reactor-mass readback with an explicit 0–150 kg scale, Alpha.Security observer/operator command enforcement, and Alpha.Reports render plus XLSX/PDF export, marker readback and temporary-template rollback. Phase 3–5 regression, Alpha.DevStudio compile/rebuild, Alpha.HMI BINOM export, real Viewer interaction and independent OPC UA readback pass in the same isolated gate. The isolated Alpha.Net/Alpha.Domain, Alpha.Server and Alpha.Historian test contour is stopped afterward, test ports are closed, standard Alpha services and `11010/11020` are restored, and the testbed worktree is clean. Passwords, hashes and tokens remain outside Git; no push, PLC, customer or production change was performed.
RATIONALE: Stanislav explicitly approved the Phase 6 Memory Candidate in Telegram topic `HOME:14` on 2026-08-25 after receiving the native alarm/history/chart/security/report results, repeatable gate, rollback status, clean worktree and commit `65d5f87`.
---
### ID: D-2026-08-25-11
TYPE: DECISION
STATUS: ACTIVE
DATE: 2026-08-25
TITLE: Accept Universal Recipe Testbed Phase 7 universality proof
CONTENT: Treat `/home/stanislav/projects/alpha-bpr-universal-testbed` commit `4fc05ce` (`feat: prove multi-mapping recipe universality`) as the accepted Phase 7 baseline. The same checked-in recipe/batch engine executes three canonical recipes against two materially different generated mappings: reactor/mixer `R-101/M-101` and flexible blending vessel `BV-201/AG-201` with 600 kg capacity, 480 rpm agitator range and 75 °C temperature-setpoint maximum. All six recipe/mapping combinations pass commands, setpoints, interlocks, expiry, duplicate/idempotency, real Alpha.HMI Viewer navigation and START/RESET readback, native alarm/acknowledgement, Alpha.Historian, alpha.hmi.charts, Alpha.Security and Alpha.Reports parity. Mapping-specific native Alpha source and HMI artifacts differ by SHA-256 while application code remains unchanged between profiles. `./scripts/run_phase7_gate.sh` reproduces the matrix and rollback. A changed mapping cannot currently be activated through the isolated Domain deploy endpoint because service executable-path lookup returns `Unknown error -1`; the accepted gate therefore uses compiled local Alpha.DevStudio build output in isolated Alpha.Server directories and does not claim deploy success. Isolated processes are stopped, test ports closed and standard Alpha services restored. No push, PLC, customer or production change was performed.
RATIONALE: Stanislav explicitly approved the Phase 7 Memory Candidate in Telegram topic `HOME:14` on 2026-08-25 after receiving the six-cell universality matrix, native artifact-difference proof, Viewer/runtime parity, rollback status, infrastructure limitation and commit `4fc05ce`.
---
### ID: D-2026-08-25-12
TYPE: DECISION
STATUS: ACTIVE
DATE: 2026-08-25
TITLE: Accept Universal Recipe Testbed Phase 8 native Modbus PLC simulator gate
CONTENT: Treat `/home/stanislav/projects/alpha-bpr-universal-testbed` commit `154aa80` (`feat: close native Modbus PLC simulator gate`) as the accepted Phase 8 baseline. The isolated PLC endpoint is a local Modbus TCP simulator bound only to `127.0.0.1:15020`, unit ID 1, and is integrated through the native Alpha.Server `ModbusTcpMaster` module. The repeatable `./scripts/run_phase8_gate.sh` proof verifies real Alpha.HMI Viewer START/RESET actions with independent OPC UA readback, TTL expiry, NOT_READY and interlock rejection, duplicate/idempotent handling, communication loss producing `BadNoCommunication`, explicit Viewer indication `BAD / НЕТ СВЯЗИ`, and automatic reconnect without restarting Viewer. Test ports are closed after the gate and standard Alpha services remain active. No external PLC, customer system, production environment or push was involved.
RATIONALE: Stanislav explicitly approved the Phase 8 Memory Candidate in Telegram topic `HOME:14` on 2026-08-25 after receiving the native Modbus transport, Viewer/readback, protective-scenario, communication-loss/reconnect, rollback, clean-worktree and commit results.
---
### ID: D-2026-08-26-01
TYPE: DECISION
STATUS: ACTIVE
DATE: 2026-08-26
TITLE: Accept Alpha BPR Universal Recipe Testbed Phase 9 pilot-readiness gate
CONTENT: Accept Phase 9 as the internal Home pilot-candidate baseline. Track F v2 is reviewed and committed in the main Alpha BPR repository at `9a899bc`; the standard Alpha.Domain deploy-path correction is committed in the testbed at `0cd0d27`, with Mapping B deployed as `0.1.0.d24`; restart/recovery/soak implementation is committed at `eeb9a08`, with evidence commits `6660c51` and `b3af2f7`. The repeatable gate proves PLC and Alpha.Server recovery without duplicate START execution, a 1800-second soak with 360/360 samples, no task-count growth and a 2.16% memory decrease, plus regression of Phases 3–8, real Viewer behavior, Historian, alarms, roles and reports. The pilot archive is reproducible across two consecutive builds with SHA-256 `26390c7fa2df2bb1c8c4db301182e1a476094235c25a12ad0420f57177cca18b`. Test services and ports are closed afterward and standard Alpha services remain active. This acceptance is an internal Home pilot candidate only; it is not customer/plant acceptance, production deployment, validation with a physical PLC, or authorization to push.
RATIONALE: Stanislav explicitly approved the Phase 9 Memory Candidate in Telegram topic `HOME:14` on 2026-08-26 after receiving the Track F v2, standard Alpha.Domain deploy, reproducible pilot package, restart/recovery, 30-minute soak, full regression, rollback, service-state and commit results.
---
### ID: D-2026-08-26-02
TYPE: DECISION
STATUS: ACTIVE
DATE: 2026-08-26
TITLE: Accept Alpha BPR Phase 9 Dark UI RC2
CONTENT: Accept `/home/stanislav/projects/alpha-bpr` tag `alpha-bpr-phase9-dark-ui-rc2` at commit `0c105f4` as the internal Home baseline for the current dark Alpha BPR interface. The accepted evidence covers the complete recipe-to-batch lifecycle through the actual dark UI and live API: recipe creation with mandatory product/profile/mixing bindings, formula and procedure, submit/approve/return/make-effective, batch snapshot/scaling, released lots and actual doses, line clearance, execution, deviation, EBR, QA disposition and release. The same UI passes native Alpha.Server OPC UA RESET/START, independent command-ID/status/error readback, exact Operator/QA role enforcement, interlock blocking, explicit `BAD / НЕТ СВЯЗИ`, disabled commands on stale communication, and automatic reconnect without page reload. Visual review passed all 11 routes on desktop and mobile (22/22). The accepted internal training set is the updated full guide, quick start, operator checklist and 91-second H.264 1280×720 dark-UI video; old July-interface footage remains superseded and excluded. Commits are `3c55f6f`, `7bc7bb3`, `846804d`, and RC manifest `0c105f4`. The verified LAN backup is `Documents/openclaw-backups/alpha-bpr/phase9-dark-ui-rc2-20260826-1730` on Synology: 10 files, 79 MiB, all SHA-256 checks and Git bundle verification PASS. This is not physical-PLC FAT, object SAT, production acceptance, QMS validation or electronic-signature approval. Two unrelated TRIZ artifacts are excluded.
RATIONALE: Stanislav explicitly approved the Memory Candidate in Telegram topic `HOME:14` on 2026-08-26 after receiving the complete current-dark-UI lifecycle/runtime/visual results, corrected documentation and video, RC tag, Synology location and integrity checks.
---
### ID: D-2026-08-26-03
TYPE: DECISION
STATUS: ACTIVE
...
STATUS: ACTIVE
DATE: 2026-09-06
TITLE: Center НППА on semantic contracts rather than a mandatory platform
CONTENT: Define the primary value of НППА as an independently governed system of semantic contracts and verifiable interoperability rather than another mandatory national technology platform. Its core assets are the unified registry of semantically aligned information models (ЕРИМ), an open federated industrial data bus, application and conformance profiles, mappings between domain standards and runtime representations, machine-readable artifacts, TCK/testbed infrastructure, a registry of conformant components, and Vendor Exit Tests. Different PLC, DCS, SCADA, historian, MES, edge, and analytics platforms conform to these contracts; no single product owns or defines the ecosystem. ЕРИМ must not become a closed centralized runtime or a single compulsory ontology: specifications and exports remain public and machine-readable, governance is independent, registry deployment may be federated, and multiple interchangeable implementations are required.
RATIONALE: Stanislav explicitly approved this Memory Candidate in Telegram topic `HOME:1751` on 2026-09-06 after agreeing that a semantic-contract ecosystem is stronger than the concept of НППА as another platform.
